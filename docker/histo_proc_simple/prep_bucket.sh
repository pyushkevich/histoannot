...
function process_dir()
{
    ...
    local PYRAMID THUMB FN_BASE FN_EXT FN_NOEXT CREDS
    unset NOCLOBBER
    JPEGLEVEL=80
    THUMBSIZE=512

    # Read optional arguments
    while getopts "nj:t:k:" flag; do
        case "$flag" in
            n) NOCLOBBER=1;;
            j) JPEGLEVEL=$OPTARG;;
            t) THUMBSIZE=$OPTARG;;
    ...
    gsutil ls $SOURCE > $SRCLIST

    # Get the target image list too
    if [[ $NOCLOBBER ]]; then
        gsutil ls $TARGET/** > $TRGLIST
    fi
    ...
    while IFS= read -r FN; do
        ...
        # Process the image
        process_image -j $JPEGLEVEL -t $THUMBSIZE \
            "$FN" "$TARGET/$FN_NOEXT/"
